...
function AchivementCard() {
  return (
    <div className="bg-zinc-900 p-5 rounded-lg flex flex-col gap-3 mx-3 max-w-lg ">
      <div className="flex items-center">
        <svg
          ...
          height="48"
          viewBox="0 0 24 24"
          className="fill-gray-300 bg-zinc-700 p-1 rounded-md"
        >
...
export default function AtheletePage({ data }) {
  ...
  return (
    <Layout>
      {athelete?.map((data) => (
        <div className="flex flex-col items-center gap-10 py-10 bg-zinc-800 text-white">
          <div className="flex flex-col items-center gap-4">
            <img
              alt="profile picture"
              src={`${ip}${data.profilePicture?.url}`}
              className="h-48 w-48 rounded-full object-cover"
            />
            <div className="flex flex-col items-center">
              <h2 className="text-2xl font-bold">{data.name}</h2>
              <h3 className="text-xl font-bold text-zinc-300">{data.sport}</h3>
            </div>
          </div>

          <div className="h-0.5 w-3/4  bg-zinc-600" />

          <div className="flex flex-col items-center gap-4">
            <h3 className="text-2xl font-bold">My Story</h3>
            <p className="px-5 text-center">{data.bio}</p>
          </div>
